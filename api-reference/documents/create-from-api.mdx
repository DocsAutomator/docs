...
<ParamField body="recipients" type="object[]">
  ...
  In the automation's Signing & Acceptance settings, a signer's email can be set to **Given with each request**. This option is available for API and MCP automations and automations without a data source. Every request must then name that signer in `recipients`, or it fails with "Signer 1 needs an email: pass it in recipients".
