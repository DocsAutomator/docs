...
<ParamField body="recipients" type="object[]">
  ...
  In the automation's Signing & Acceptance settings, a signer's email can be set to **Given with each request**. Every request must then name that signer in `recipients`, or it fails with "Signer 1 needs an email: pass it in recipients".
