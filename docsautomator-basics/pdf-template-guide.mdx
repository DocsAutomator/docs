...
<Steps>
  ...
  <Step title="Go to the template area">
    PDFs are added directly in the template area. There is no separate PDF template type to choose; uploading or dropping a PDF makes it your template.

    <Frame caption="The template area, where you add a PDF by uploading it">
      <img src="/images/pdf-template/template-type-selection-new.png" alt="Template area with a drag and drop zone that accepts PDF, Word and Google Docs files" />
    </Frame>
  </Step>
...
<Frame caption="Three-dots menu on the template row showing Replace PDF file and Switch template format">
  <img src="/images/pdf-template/replace-pdf-menu.png" alt="Three-dots menu open with Replace PDF file and Switch template format options" />
</Frame>
...
<Note>
Chinese, Japanese, and Korean text renders with Noto Sans CJK in every font family. You do not need to select a special font.
</Note>
...
Each e-sign field is assigned to a signer by number. The field name includes the signer number (e.g., `esign_signature_1`, `esign_date_2`). Configure signer details (name, email) in the [E-Signing card](/features/docsautomator-esign) of your automation.
