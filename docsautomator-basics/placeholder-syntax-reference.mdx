...
<Info>
This `{{...}}` syntax applies to **Google Doc templates** only, where the brackets sit directly in the document and get replaced at generation time. **PDF templates do not use this syntax.** Don't type `{{...}}` into a PDF; use the [visual editor](/docsautomator-basics/pdf-template-guide) to position fields instead.
</Info>
