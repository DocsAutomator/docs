...
| [Dynamic images](/features/dynamic-images) | Yes | Any placeholder whose name contains `image`; one image per placeholder, so for multi-image fields pick "First image only" or "Last image only" in the [image options](/features/dynamic-images#choosing-which-image-to-insert) |
