...
<Frame caption="The template library filtered to Word designs">
  <img src="/images/word-template/template-type-picker.png" alt="Template library with the Word format filter selected, showing ready-made Word designs by category" />
</Frame>
...
<Steps>
  ...
  <Step title="Review detected placeholders">
    ...
    <Frame caption="DocsAutomator shows the file name, the number of placeholders detected, and a live preview">
      <img src="/images/word-template/uploaded-docx.png" alt="Word template in the automation editor showing the file name, the placeholder count, and a live preview of the invoice" />
    </Frame>
  </Step>
</Steps>
...
Type these tags where the signing elements should appear, then configure signers in the [E-Signing card](/features/docsautomator-esign) of your automation.
...
## Downloading Your Word Template

You can take your `.docx` back out at any time, including every change you or the AI made to it inside DocsAutomator.

In the automation, find the **template card** (the panel showing your template, with the **Edit Template** button). Click the **⋮** button next to *Edit Template* and choose **Download Word file**.
...
Once a Word template is attached, the template card's **⋮** menu gives you these controls:

| Action | What happens |
|--------|--------------|
| **Download Word file** | Downloads the current `.docx`, including edits made inside DocsAutomator. |
| **Replace Word file** | Upload a new `.docx` in place of the current one. Placeholders are re-detected from the new file. |
| **Switch template format** | Returns you to the template-type picker so you can start over (or switch to Google Docs or PDF). |
