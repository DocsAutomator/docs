...
Airtable, Google Sheets, SmartSuite and ClickUp automations have no Format option. Format those values in the source instead, for example with a formula field in Airtable that returns Yes when a checkbox is ticked. Google Forms automations show the Format option only on scale, rating, score and date answers, so they have no True / False style.
