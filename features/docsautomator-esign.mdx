...
Signing has an explicit on/off switch. Open the **E-Signing** card of your automation and turn on **Signing**. While signing is off, the panel shows only the switch, and generated documents are delivered without a signing request.

- Adding `{{esign.*}}` tags to the template does not switch signing on. Removing them does not switch it off.
- When the template has signing tags and signers are set, but signing is off, the E-Signing card shows **Off · template has signing tags**.
- Through the API or MCP, set `esignature.enabled` to `true` or `false`. Setting signers for the first time switches it on.
