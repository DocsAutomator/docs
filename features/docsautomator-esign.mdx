...
<Info>
The `sourceData` field contains the original data from your data source that was used to generate the document. The `additionalParams` field contains any `webhookParams` you passed in the API request.

For Airtable and SmartSuite automations, the record ID (`recId`) is automatically included in `additionalParams`, so you can identify which record the e-sign session belongs to.
</Info>
...
<Info>
The `sourceData` field contains the original data from your data source that was used to generate the document. The `additionalParams` field contains any `webhookParams` you passed in the API request. For Airtable and SmartSuite automations, the record ID (`recId`) is automatically included in `additionalParams`.
</Info>
