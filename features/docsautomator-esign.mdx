...
description: "Close a generated document with an e-signature, a one-click acceptance, or no closing step"
icon: "signature"
keywords: ["electronic signatures", "e-signatures", "acceptance", "click to accept", "document signing", "legally binding", "ESIGN Act", "eIDAS"]
---

Every automation has a **closing step**. It decides what happens after DocsAutomator generates the document. You choose one of three options in the **E-Signing** card of the automation, under **Closing**.

| Closing step | What the recipient does | Use it for |
|--------------|-------------------------|------------|
| **Signature required** | Opens the document, fills the signature fields from your template, and clicks Complete | Contracts, agreements, and any document where the law or your policy asks for a signature |
...
| **None** | Nothing. The document is delivered without a closing step | Invoices, reports, certificates, and any document that needs no reply |
...
## Choosing the Closing Step

Open the **E-Signing** card of your automation. Under **Closing**, pick **Signature required**, **Acceptance**, or **None**. While the closing step is **None**, the card shows only the choice, and generated documents are delivered without a signing or acceptance request.

- Adding `{{esign.*}}` tags to the template does not change the closing step. Removing them does not change it either.
- When the template has signing tags but the closing step is **Acceptance**, the tags are left blank in the document. The card tells you so.
- Through the API or MCP, set `esignature.enabled` to `true` or `false`, and `esignature.closeType` to `signature` or `acceptance`. Setting signers for the first time switches signing on. Two more fields apply to acceptance only: `esignature.acceptanceRequireName` (default `true`) and `esignature.acceptancePaymentObligation` (default `false`). The [acceptance page](/features/docsautomator-esign/acceptance#set-it-up-through-the-api-or-mcp) explains them.
...
2. Configure all signers in the E-Signing card. Only signers whose fields appear in the generated document are included in the session.
...
Set this in the automation's **E-Signing** panel, under **After signing → Save Signed PDF To**. It is a separate setting from the PDF write-back in the Actions card, which saves the unsigned document at generation time.
...
Fires once per e-sign email (invitation, resend, reminder, completion, progress, decline, expiration) with the outcome reported by the mail provider.
