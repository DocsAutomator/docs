...
<Info>
The `sourceData` field contains the original data from your data source that was used to generate the document. The `additionalParams` field contains any `webhookParams` you passed in the API request. Both are included so you can identify which record the declined session belongs to.
</Info>
