  ...
  "sessions": [
    {
      ...
      "webhookParams": { "recId": "recAbC123XyZ" },
...
`closeType` tells you how the session closes: `signature` (the default) or `acceptance`. An [acceptance](/features/docsautomator-esign/acceptance) session has no `esignFields`. A recipient who accepted has the signer status `signed`, and the signer object carries `typedName` when the recipient typed their name before accepting. Both the list and the detail endpoints return `closeType`.
...
| `document_accepted` | Recipient clicked Accept on an acceptance session (`metadata.typedName` holds the typed name) |
...
| `email_sent` | Completion, progress, decline or expiration email accepted by a provider |
| `email_send_failed` | Any e-sign email ended with outcome `failed` or `unknown` |

`invite_sent`, `invite_resent`, `reminder_sent`, `email_sent` and `email_send_failed` carry the delivery record in `metadata.delivery`. See [Email Delivery Outcomes](/features/docsautomator-esign/email-delivery-outcomes).
...
| `esign.email_sent` | An e-sign email was accepted by a mail provider |
| `esign.email_failed` | An e-sign email ended with outcome `failed` or `unknown` |
...
<Note>
For the final signer, `progress.allSigned` is `true` but the session `status` is still `in_progress`. The separate `Session Completed` webhook fires afterwards, once the signed PDF is ready.
</Note>
...
Parameters are returned in all webhook notifications under `webhookParams`, together with the automatically included trigger record identifier (`recId`, `taskId`, or `rowNumber` depending on the data source). An identical `additionalParams` key is also present — it exists only for legacy reasons, so use `webhookParams` in new integrations.
...
| `invited` | Email sent |
| `opened` | Link opened |
| `signed` | All fields completed, or the document accepted |
| `declined` | Declined to sign or to accept |

### Acceptance Sessions

Sessions with `closeType: "acceptance"` use the same session and signer statuses. A recipient who clicked Accept has the signer status `signed`. The session is `completed` when every recipient has accepted. The audit trail records the click as a `document_accepted` event, with the typed name in `metadata.typedName` when the recipient typed one.
...
def handle_esign_webhook():
    ...
    if payload.get('event') == 'esign.session_created':
        ...
        order_id = payload.get('webhookParams', {}).get('orderId')
