...
<Tabs>
  <Tab title="From Automation Interface">
    Once your automation has a template and a connected data source, click the **Preview** button in the top bar to open a preview of your document.
  ...
  <Tab title="Test Mode for Entire Automation">
    Set the automation's status to **Test Mode** to test the complete workflow:
    ...
    - If "Save PDF in Google Drive" is enabled, saves the watermarked PDF to your Drive folder
    ...
    - Emails are still sent, if the automation has an email action enabled

    <Warning>
    Test Mode does not stop emails. Only the **Preview** button inside the app skips the email action. A run triggered by the API, Airtable, a webhook or polling sends the configured email, even in Test Mode. Turn the email action off, or send to a test address, while you test.
    </Warning>
...
| Emails Sent | Yes (if configured) | Yes (if configured) |
