...
<Tabs>
  ...
  <Tab title="API, Zapier, Make, n8n, Glide, Noloco, MCP">
    These sources have no field list, so point the link at another mapped placeholder that holds the URL. Map the URL to a placeholder, select it as the link source, and hide that placeholder so it does not also render as text.
  </Tab>
</Tabs>
