...
<Tip>
Attachment fields often collect photos over time. Since new files are added at one end of the list, "First image only" and "Last image only" let you reliably pick the newest or the oldest image, no matter how many are attached.
</Tip>

<Note>
Word templates insert a single image per placeholder: "First image only" and "Last image only" work as described, while a multi-image field left on "All images" is not rendered. Google Doc templates insert all images inline.
</Note>
