...
### Reusing a section name

The same section name can be used as many times as you like in one template. Each pair of tags is treated as its own block, and the single rule you configure for that name controls every copy, so they all show or hide together.
...
{{section_vendor_2}}
Vendor 2 signature block
{{/section_vendor_2}}
```

This is useful when the same clause, signature block or table column appears in more than one place in a long document. To hide a table column, wrap both its header cell and its data cell in a section with the same name.

<Note>
Copies of a section cannot be given different conditions. If two blocks need to appear independently, give them different names.
</Note>

### Nesting sections

A section can sit inside another section. Each one keeps its own rule: the inner block shows only when both its own rule and the outer rule pass. Hiding the outer section removes everything between its tags, inner sections included.

```
{{section_eu_customer}}
EU terms apply.
{{section_vat_registered}}
VAT number: {{vatNumber}}
{{/section_vat_registered}}
{{/section_eu_customer}}
```

Close the inner section before the outer one. Nesting works in Google Doc and Word templates.
...
It also enables a **variable number of signers** from one template: wrap each optional signer's block in its own section, and only the signers whose blocks are shown become part of the signing session. Signer numbers need not be consecutive — a signer who always signs (such as a countersigning agent) can keep a fixed high number (`{{esign.signature_7}}`) outside any section, so their block never moves as optional signers come and go. Sequential signing follows the signers present, in number order. See [Variable Number of Signers](/features/docsautomator-esign#variable-number-of-signers).
