...
Pick a field and a value that should fire generation. When a record changes to match (for example, **Status = Approved**), DocsAutomator generates a document for that record, usually within seconds. Nothing needs to be set up inside Airtable.

Trigger field types supported:
- **Single select**
- **Multiple select** (fires when the chosen option is among the selected values)
- **Checkbox**

Records that are created already matching the condition also fire, so imports and bulk-added rows are covered. Renaming the select option in Airtable does not break the trigger.
