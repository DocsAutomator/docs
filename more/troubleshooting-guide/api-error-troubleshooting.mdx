...
```json
{ "message": "Please add a valid automation id with key 'automationId' (or 'docId') to the request." }
```
Include the `automationId` field in your request body with your automation ID.

**Different automation IDs in one request:**
```json
{ "message": "The request has both 'automationId' and 'docId' with different values. Send only 'automationId'." }
```
Send the automation ID once, in `automationId`. `docId` is still accepted on its own.
...
|-------|-------------|----------|
| `"Automation / template id does not exist or is not part of the account with the given API key."` | Wrong `automationId` or wrong workspace | Verify the automation ID and API key match the same workspace |
