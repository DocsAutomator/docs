...
<Info>
This page uses Google Docs for its examples. The placeholder rules below apply to [Word (.docx) templates](/docsautomator-basics/word-template-guide) as well, since both use the same `{{placeholder}}` syntax. [PDF templates](/docsautomator-basics/pdf-template-guide) work differently: fields are positioned visually rather than typed into the page.
</Info>
...
1. **Missing table row**: Line item placeholders must be placed inside a table row. They cannot be used in regular paragraph text. This applies to [Word templates](/docsautomator-basics/word-template-guide) too: a line item marker outside a table row is dropped.
